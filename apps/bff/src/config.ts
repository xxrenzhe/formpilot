...
export const env = {
  ...
  adsOnlyMode: asBoolean(process.env.ADS_ONLY_MODE, true),
  freeSignupCredits: Number(process.env.FREE_SIGNUP_CREDITS || 20),
  trialIpClaimWindowHours: Number(process.env.TRIAL_IP_CLAIM_WINDOW_HOURS || 24),
  trialIpClaimMaxPerWindow: Number(process.env.TRIAL_IP_CLAIM_MAX_PER_WINDOW || 4),
  promptAutoWeightEnabled: asBoolean(process.env.PROMPT_AUTO_WEIGHT_ENABLED, false),
  promptWeightDeltaAccept: Number(process.env.PROMPT_WEIGHT_DELTA_ACCEPT || 0.01),
  promptWeightDeltaReject: Number(process.env.PROMPT_WEIGHT_DELTA_REJECT || -0.02),
